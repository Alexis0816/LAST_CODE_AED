...
private:
  ...
  {
    return (nodo == nullptr) ? -1 : 1 + max(height(nodo->left), height(nodo->right));
  }
  ...
  void insert(Node *&root, T data)
  {
    ...
    {
      (root->data < data) ? insert(root->right, data) : insert(root->left, data);
    }
  }
...
{
  BSTree<int> BST;
  BST.insert({8,3,10,1,6,4,7,14,13});
